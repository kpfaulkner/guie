...
func TestColorPickerClickThroughHarness(t *testing.T) {
	h := guitest.New(220, 140)
	var got color.Color
	cp := ui.NewColorPicker(ui.ColorPickerValue(color.NRGBA{R: 255, A: 255}))
	...
	if h.Frame().Count(guitest.OpFillRect) < 10 {
		t.Fatal("color picker should draw gradient strips")
	}
	...
	nc := color.NRGBAModel.Convert(cp.Color()).(color.NRGBA)
