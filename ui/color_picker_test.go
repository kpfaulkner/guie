...
func TestColorPickerClickSetsHue(t *testing.T) {
	...
	if fired == 0 {
		t.Fatal("OnChange should fire on a click that changes the color")
	}
}

func TestColorPickerKeyboard(t *testing.T) {
	p := NewColorPicker(ColorPickerValue(color.NRGBA{R: 255, G: 255, B: 255, A: 255})) // h0 s0 v1
...
func TestColorPickerSetColorFiresOnChange(t *testing.T) {
	...
	p.SetColor(color.NRGBA{R: 255, A: 255}) // unchanged
	p.SetColor(color.NRGBA{B: 255, A: 255}) // change to blue
	if calls != 1 {
